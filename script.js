...
function initDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('ProfilesDB', 1);

    request.onupgradeneeded = (event) => {
      const db = event.target.result;
      if (!db.objectStoreNames.contains('profiles')) {
        const store = db.createObjectStore('profiles', { keyPath: 'id', autoIncrement: true });
...
// Fonction pour ajouter un profil à IndexedDB
function addProfile(db, profile) {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      reject('Erreur lors de l\'ajout du profil.');
    };
  });
}

// Fonction pour récupérer tous les profils depuis IndexedDB
function getAllProfiles(db) {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      reject('Erreur lors de la récupération des profils.');
    };
  });
}
...
function deleteProfile(db, id) {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      reject('Erreur lors de la suppression du profil.');
    };
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
      ...
      fetch(nominatimUrl)
        ...
        .then(data => {
          if (data && data.length > 0) {
            ...
            // Forcer la position et l'affichage après ajout sur mobile
            if (window.innerWidth <= 480) {
              setTimeout(() => {
                if (topLeftControls) {
  ...
  // Gestion du profil (conversion image en base64 et stockage dans IndexedDB)
  ...
  if (profileForm) {
    profileForm.addEventListener('submit', async (e) => {
      ...
      try {
        await addProfile(db, newProfile);
    ...
    profileFilterBtn.addEventListener('click', async () => {
      ...
      if (profiles.length > 0) {
        profiles.forEach((profileData) => {
          const { id, name, description, photo } = profileData;
          ...
          singleProfileContainer.style.flexWrap = 'wrap'; // Pour une meilleure adaptation sur mobile
          ...
          textContainer.style.flex = '1'; // Prendre tout l'espace disponible
